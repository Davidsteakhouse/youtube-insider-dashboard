...
Telegram 경쟁 브리프는 조회수로 전체 채널을 한 줄 정렬하지 않고 `AI 핵심 채널의 AI 영상 →
인접 채널의 명시적인 AI 영상 → 남은 자리만 일반 테크 영상` 순서로 최대 10개를 보냅니다.
따라서 잇섭·테크몽·주연·귀곰 같은 대형 테크 채널은 조회수가 높아도 AI 후보보다 앞에 오지
않으며, AI 후보가 10개에 못 미칠 때만 보충됩니다. 비AI 비즈니스 영상과 미분류 채널은
보충하지 않습니다. 각 항목에는 제목뿐 아니라 자막·설명 기반 한국어 내용 요약과 영상 링크가
포함됩니다. 실제 요약을 만들지 못한 영상은 빈 문구나 제목만 보내지 않고 다음 후보로
대체합니다.
