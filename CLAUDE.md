...
7. Telegram 경쟁 브리프는 `AI 핵심 → 인접 채널의 AI 콘텐츠 → 일반 테크 보충` 순서로 사용한다.
   - 조회수는 같은 우선순위 안에서만 정렬한다.
   - `테크`, `IT` 채널의 일반 영상은 AI 후보가 10개 미만일 때만 남은 자리를 채운다.
   - `비즈니스/AI` 채널의 비AI 영상과 미분류 채널은 보충하지 않는다.
   - 최대 10개를 보내고 후보가 10개 미만이면 실제 수량만 보낸다.
...
- AI 핵심·인접 AI 콘텐츠를 먼저 배치하고, 10개 미만일 때만 일반 테크 영상으로 보충
